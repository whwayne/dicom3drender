 ...
 * @author fuanka
 */
public class nkDicomFileUtil {
    public static boolean fileContainsMultipleFrames(File aDicomFile) throws IOException,DicomException{
        AttributeList al = new AttributeList();
        al.read(aDicomFile.getPath());
        SourceImage si=new SourceImage(al);
        if(si.getNumberOfFrames()>1)
            return true;
        else
            return false;
    }

     public static boolean fileContainsMultipleFrames(AttributeList al) throws IOException,DicomException{
        SourceImage si=new SourceImage(al);
        if(si.getNumberOfFrames()>1)
            return true;
        else
